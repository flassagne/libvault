...
public:
  ...
  static std::string encode(std::string value) {
    return encode(reinterpret_cast<const unsigned char *>(value.c_str()),
                  value.length());
  }
...
public:
  ...
  optional<HttpResponse> get(const std::string &url, const std::string &string,
                             const std::string &ns) const;
  optional<HttpResponse> post(const std::string &url, const std::string &token,
                              const std::string &ns, std::string value) const;
  optional<HttpResponse> del(const std::string &url, const std::string &token,
                             const std::string &ns) const;
  optional<HttpResponse> list(const std::string &url, const std::string &token,
                              const std::string &ns) const;
...
private:
  ...
  executeRequest(const std::string &url, const std::string &token,
                 const std::string &ns,
                 const CurlSetupCallback &callback) const;
};
...
private:
  VaultConfig()
      : tls_(true), debug_(false), verify_(true), connectTimeout_(10),
        host_("localhost"), port_("8200"), ns_("") {}
  bool tls_;
  bool debug_;
  bool verify_;
  long connectTimeout_;
...
public:
  ...
  VaultConfig &build() { return config_; }

  explicit operator VaultConfig &&() { return std::move(config_); }
...
public:
  static optional<std::string> post(const VaultClient &client,
...
public:
  ...
  optional<std::string>
  authenticate(const VaultClient & /*vaultClient*/) override {
    return token_;
  }
...
public:
  ...
  ~AppRole() override = default;

  optional<std::string> authenticate(const VaultClient &vaultClient) override;

private:
  static std::string getUrl(const VaultClient &vaultClient,
...
class Unwrap {
public:
  static optional<std::string> unwrap(const VaultClient &client,
                                      std::string token);
...
public:
  ...
  ~WrappedSecretAppRole() override = default;
...
class Ldap : public AuthenticationStrategy {
public:
  Ldap(std::string login, std::string password);
  ~Ldap() override = default;

  optional<std::string> authenticate(const VaultClient &client) override;
...
public:
  ...
  optional<std::string> put(std::string path,
...
public:
  ...
  optional<std::string> generate_data_key(std::string path,
                                          Parameters parameters);
  optional<std::string> generate_wrapped_data_key(std::string path,
                                                  Parameters parameters);
  optional<std::string> generate_random_bytes(int num_bytes,
                                              Parameters parameters);
  optional<std::string> hash(std::string algorithm, Parameters parameters);
  optional<std::string> hmac(std::string key, std::string algorithm,
                             Parameters Parameters);
  optional<std::string> sign(std::string key, std::string algorithm,
                             Parameters Parameters);
  optional<std::string> verify(std::string key, std::string algorithm,
                               Parameters Parameters);
